...
class table_6_8(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.s1 = []
        self.checkbox = []
        self.s1.append(QTextEdit('바. 회복조치를 수행하도록 주제어실에 지시한다.'))
        self.s1.append(QTextEdit('&nbsp;&nbsp;&nbsp;&nbsp;• 전략이 수행된 후 그 전략이 장기간 지속되는 것을 확실하게 하기 위하여 취해야 하는 조치'))
        self.s1.append(QTextEdit('&nbsp;&nbsp;&nbsp;&nbsp;• 전략에서 요구하는 기능을 수행하기 위한 계통의 재배열'))
        self.s1.append(QTextEdit('&nbsp;&nbsp;&nbsp;&nbsp;• 현재 전략을 종료하고 대체 전략을 수행하기 위한 계통의 재배열'))

        for i in range(4):
            self.s1[i].setDisabled(True)
            self.s1[i].setFixedHeight(40)  # QTextEdit 때문에 설정해줘야함 (addStretch 안먹음)
        self.s1[1].setFixedHeight(80)
