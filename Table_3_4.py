...
class table_3_4(QWidget):
    ...
    def __init__(self, parent=None):
        super(table_3_4, self).__init__()
        self.setAttribute(Qt.WA_StyledBackground, True)
        self.setContentsMargins(0, 0, 0, 0)
        self.setStyleSheet(self.qss)
        # 기본 속성
        layout = QVBoxLayout(self)
        label = QTextEdit("4.나. 부정적 영향이 문제되지 않는다고 판단되면 단계 6으로 간다.")
        label.setStyleSheet("font-size: 18pt;font-weight: bold")
        label.setContentsMargins(10, 10, 10, 20)
        label.setDisabled(True)
        label.setFixedHeight(45)  # QTextEdit 때문에 설정해줘야함 (addStretch 안먹음)
