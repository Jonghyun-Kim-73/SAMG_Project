 ...
 QTableWidget{
  border: 0px solid rgb(0,0,0);     
    ...
    def __init__(self):
        ...
        # 기본 속성
        layout = QVBoxLayout(self)
        label = QLabel("6. 증기발생기 급수 주입 경로를 선정한다.")
        label.setStyleSheet("font-size: 18pt;font-weight: bold")
        label.setContentsMargins(10, 10, 10, 20)

        label1 = QLabel("<p style=\"line-height:130%\">가. 급수 주입을 할 증기발생기는 다음 순서에 따라 선정한다.<p>"
                        "<p style=\"line-height:130%\">1) 건전한 증기발생기(들)<p>"
                        "<p style=\"line-height:130%\">2) 고장난 증기발생기(들)<p>"
                        "<p style=\"line-height:130%\">3) 튜브가 파손된 증기발생기(들)<p>")
        label1.setStyleSheet("font-size: 14pt;font-weight: bold")
        label1.setContentsMargins(10, 10, 10, 20)

        label2 = QLabel("<p style=\"line-height:130%\">나. 선정된 계통의 배열을 확인한다.<p>"
                        "<p style=\"line-height:130%\">1) 펌프<p>"
                        "<p style=\"line-height:130%\">2) 펌프 전단 배열 상태<p>"
                        "<p style=\"line-height:130%\">3) 펌프 후단 배열 상태<p>")
        ...
        para_table2 = ParaTable2(self)
        para_table3 = ParaTable3(self)

        layout.addWidget(label)
        layout.addWidget(label1)
        layout.addWidget(para_table)
        layout.addWidget(label2)
        layout.addWidget(para_table2)
        layout.addWidget(para_table3)
        layout.addStretch()
...
class ParaTable2(QTableWidget):
    def __init__(self, parent):
        ...
        self.setItem(0, 0, QTableWidgetItem("이용가능한 펌프"))
...
class ParaTable3(QTableWidget):
    def __init__(self, parent):
        ...
        self.setItem(0, 0, QTableWidgetItem("이용가능한 배관"))
