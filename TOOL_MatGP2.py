...
class Trend(QWidget):
    def __init__(self, parent, w, h, para_name: str, para_id: str,
        ...
        self.progline, = self.ax.plot([], [], color=[39/255, 39/255, 141/255], linewidth=1, linestyle='--')    # 예지
        ...
        if self.parent is not None:
            ...
            * Required functions of graph
            ...
            if len(time_db) > 2:    # 2개 이상 포이트가 저장되면 드로잉 시작
                ...
                if _shmem['KCNTOMS'][-1] > 1 * 5:
                    if saved_db != {}:  # 예지된 값이 있는 경우 self.prog_lines 에 추가 해야함.
                        saved_db = saved_db[self.para_id]
                        _temp_x = [i * 1 for i in range(len(saved_db))]
                        self.progline.set_data(_temp_x, saved_db)

                # 압력 그래프 드로잉

                max_val = time_db[-1]
                time_db = [_ - max_val for _ in time_db]
